...
def delete(user_id: int, project_id: str, db: Session):
    ...
    entry = db.query(models.UserProject).where(models.UserProject.fk_project_id == project_id)
    if not entry.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"User id: {user_id}, project id: {project_id} not found in database!")
    try:
...
def share(user_id_sharer: int, user_id_beneficiary: int, project_id: str, db: Session):
    ...
    new_entry = models.UserProject(fk_user_id=user_id_beneficiary, fk_project_id=project_id,owner=False)
    try:
        db.add(new_entry)
        db.commit()
...
def share_exposed(current_user_email: str, beneficiary_email: str, project_id: str, db: Session):
...
def user_cancel_share(current_user_email: str, beneficiary_email: str, project_id: str, db: Session):
    #check if the project exists
    project_object = project.get_by_id(project_id, db)
    if not (project_object.is_private):
       raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def check_shared(user_id_beneficiary: int, project_id: str, db: Session):
    entry = db.query(models.UserProject).where(and_(models.UserProject.fk_project_id == project_id, models.UserProject.fk_user_id == user_id_beneficiary, models.UserProject.owner == False)).first()
    if not entry:
...
def check_shared_entry(user_id_beneficiary: int, project_id: str, db: Session):
    ...
    entry = db.query(models.UserProject).where(and_(models.UserProject.fk_project_id == project_id, models.UserProject.fk_user_id == user_id_beneficiary, models.UserProject.owner == False))
    if not entry.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def shared_projects(user_id: int, db: Session):
    ...
    #get entries where user is the owner from UserProject
    userproject = db.query(models.UserProject, models.Project, models.User).where(models.UserProject.fk_user_id == user_id).where(models.UserProject.owner == False).outerjoin(models.Project).outerjoin(models.User).all()
    if not userproject:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def create_project_user_entry(user_id: str, project_id: int, db: Session):
    new_entry = models.UserProject(fk_user_id=user_id, fk_project_id=project_id,owner=True)
    try:
        db.add(new_entry)
        db.commit()
        db.refresh(new_entry)
...
def check_beneficiaries(project_id: str, current_user_email: str, db: Session):
    #check credentials
    #check if owner or admin
    if not ((user.is_admin_bool(current_user_email, db)) or (is_owner_bool(current_user_email, project_id, db))):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
